...
import { getWindSpeedUnit, getUnitsSetting } from '../store/units';

export const getActivityQuality = (activityId: string, windSpeed: number, humidity: number, aqi: number | null): string => {
  switch (activityId) {
    case 'wind':
  ...
  switch (activity.id) {
    case 'wind': {
      ...
      return {
        ...
        quality: getActivityQuality('wind', weatherData.wind.speed, 0, null),
      };
    }

    case 'air_quality': {
      const quality = getActivityQuality('air_quality', 0, 0, aqi);
      return {
    ...
    case 'humidity': {
      return {
        ...
        quality: getActivityQuality('humidity', 0, weatherData.main.humidity, null),
      };
    }
